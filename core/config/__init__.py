...
从 api/config/ 下的 JSON 配置文件加载所有配置，并提供统一的访问接口。
...
# 默认配置目录（指向 api/config/）
_DEFAULT_CONFIG_DIR = Path(__file__).resolve().parent.parent.parent / "api" / "config"
...
# 加载所有配置
_generator_config = load_generator_config()
_embedder_config = load_embedder_config()
_repo_config = load_repo_config()
_lang_config = load_lang_config()

# 更新全局配置
if _generator_config:
    _configs["default_provider"] = _generator_config.get("default_provider", "dashscope")
    _configs["providers"] = _generator_config.get("providers", {})

if _embedder_config:
    for key in ["embedder", "embedder_ollama", "embedder_google", "embedder_bedrock", "retriever", "text_splitter"]:
        if key in _embedder_config:
            _configs[key] = _embedder_config[key]

if _repo_config:
    for key in ["file_filters", "repository"]:
        if key in _repo_config:
            _configs[key] = _repo_config[key]

if _lang_config:
    _configs["lang"] = _lang_config
...
def get_embedder_config() -> dict:
    ...
    if embedder_type == "bedrock" and "embedder_bedrock" in _configs:
        return _configs.get("embedder_bedrock", {})
    elif embedder_type == "google" and "embedder_google" in _configs:
        return _configs.get("embedder_google", {})
    elif embedder_type == "ollama" and "embedder_ollama" in _configs:
        return _configs.get("embedder_ollama", {})
    else:
        return _configs.get("embedder", {})
...
def get_model_config(provider: str = "google", model: Optional[str] = None) -> dict:
    ...
    """
    if "providers" not in _configs:
        raise ValueError("Provider configuration not loaded")

    provider_config = _configs["providers"].get(provider)
