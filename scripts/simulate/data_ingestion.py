"""
data_ingestion.py — 数据摄取管道：代码仓库 → PostgreSQL + pgvector
=====================================================================
...
数据流：
  1. 下载仓库 (git clone) 到 DATA_SOURCE 根目录
  2. 读取文档 (read_all_documents)
  3. 创建/获取项目记录 (ProjectRepository.get_or_create)
  4. 创建摄取任务 (IngestionJobRepository.create)
  5. 分块 + 嵌入 + 存储 (IngestionPipeline.process_documents)
  6. 更新任务状态
...
class DataIngestor:
    ...
    完整流程：
      ...
      3. ingest()      — 读取文档 → 分块 → 嵌入 → 存储
      4. finalize()    — 更新任务状态，返回统计信息
    """
